...
from nnunetv2.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss, HybridLoss
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda'), use_hybrid_loss = False):
    ...
    def printHyperParameters(self):
        params = {
            ...
            "use_hybrid_loss": self.use_hybrid_loss
        }

        # 打印字典
        for key, value in params.items():
            self.print_to_log_file(f"{key}: {value}")
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.network = self.build_network_architecture(self.plans_manager,
                                                           self.dataset_json,
            ...
            self.loss = self._build_loss(self.use_hybrid_loss)
    ...
    def _build_loss(self, is_hybrid = False):
        if is_hybrid:
            loss = HybridLoss(weight_main = 1, weight_aux = 0,soft_dice_kwargs={'batch_dice': self.configuration_manager.batch_dice,
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            if self.use_hybrid_loss:
                l, main_l, aux_l = self.loss(output, target)
            else:
        ...
        if self.use_hybrid_loss:
            return {'loss': l.detach().cpu().numpy(),
                'main_loss':main_l.detach().cpu().numpy(),
                'aux_loss': aux_l.detach().cpu().numpy()}

        return {'loss': l.detach().cpu().numpy()}

    def on_train_epoch_end(self, train_outputs: List[dict]):
        # 汇总 train_outputs 中的数据
        ...
        if self.is_ddp:
            ...
            if self.use_hybrid_loss:
                # 收集 'main_loss' 数据
                main_losses_tr = [None for _ in range(dist.get_world_size())]
                dist.all_gather_object(main_losses_tr, outputs['main_loss'])
                main_loss_here = np.vstack(main_losses_tr).mean()

                # 收集 'aux_loss' 数据
                aux_losses_tr = [None for _ in range(dist.get_world_size())]
                dist.all_gather_object(aux_losses_tr, outputs['aux_loss'])
                aux_loss_here = np.vstack(aux_losses_tr).mean()
        else:
            # 非分布式情况下的平均计算
            loss_here = np.mean(outputs['loss'])
            if self.use_hybrid_loss:
                main_loss_here = np.mean(outputs['main_loss'])
                aux_loss_here = np.mean(outputs['aux_loss'])

        # 记录日志
        self.logger.log('train_losses', loss_here, self.current_epoch)
        if self.use_hybrid_loss:
            self.logger.log('train_main_losses', main_loss_here, self.current_epoch)
            self.logger.log('train_aux_losses', aux_loss_here, self.current_epoch)
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            if type(output) != list:
                output=[output]
            if self.use_hybrid_loss:
                l,_,__ = self.loss(output, target)
            else:
                l = self.loss(output, target)
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        ious = [tp[i] / (tp[i] + fp[i] + fn[i]) for i in range(len(tp))]

        self.logger.log('ious_per_class', ious, self.current_epoch)
    ...
    def on_epoch_end(self):
        ...
        if self.use_hybrid_loss:
            main_loss = np.round(self.logger.my_fantastic_logging['train_main_losses'][-1], decimals=4)
            aux_loss = np.round(self.logger.my_fantastic_logging['train_aux_losses'][-1], decimals=4)
            self.print_to_log_file('main_loss', main_loss)
            self.print_to_log_file('aux_loss', aux_loss)
        ...
        ious_per_class = [np.round(i, decimals=4) for i in self.logger.my_fantastic_logging['ious_per_class'][-1]]

        self.print_to_log_file('Pseudo dice', pseudo_dice)
        self.print_to_log_file('ious_per_class', ious_per_class)
        ...
        if (current_epoch + 1) % self.save_every == 0:
            print(f"saving model{current_epoch}.pth")
            self.save_checkpoint(join(self.output_folder, f'checkpoint_epoch{current_epoch}.pth'))
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            # the validation keys across the workers.svs vswd
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            # 动态切换损失函数
            if epoch > self.epoch_switch and self.use_hybrid_loss:
                progress = (epoch - self.epoch_switch) / (self.num_epochs-self.epoch_switch)
                weight_main = math.cos(progress * math.pi / 2)
                weight_aux = 1.0 - weight_main
                self.loss.change_weight(weight_main=weight_main,
                                        weight_aux=weight_aux)

            self.on_train_epoch_start()
            train_outputs = []
            pbar = tqdm(range(self.num_iterations_per_epoch),desc="training")
            for batch_id in pbar:
                loss = self.train_step(next(self.dataloader_train))
            ...
            with torch.no_grad():
                ...
                val_outputs = []
                for batch_id in tqdm(range(self.num_val_iterations_per_epoch),desc="validation"):

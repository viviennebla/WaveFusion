...
def maybe_load_checkpoint(nnunet_trainer: nnUNetTrainer, continue_training: bool, validation_only: bool,
                          pretrained_weights_file: str = None, validation_epoch: int = None):
    if continue_training:
        ...
        if validation_epoch is not None:
            # 使用 validation_epoch 构造 expected_checkpoint_file
            expected_checkpoint_file = os.path.join(nnunet_trainer.output_folder, f"checkpoint_epoch{validation_epoch}.pth")
            print(f"loading the {validation_epoch}th checkpoint from {expected_checkpoint_file}")
        else:
            expected_checkpoint_file = join(nnunet_trainer.output_folder, 'checkpoint_final.pth')
        if not isfile(expected_checkpoint_file):
...
def run_ddp(rank, dataset_name_or_id, configuration, fold, tr, p, use_compressed, disable_checkpointing, c, val, pretrained_weights, npz, world_size, validation_epoch):
    ...
    maybe_load_checkpoint(nnunet_trainer, c, val, pretrained_weights, validation_epoch)
...
def run_training(dataset_name_or_id: Union[str, int],
                 ...
                 device: torch.device = torch.device('cuda'),
                 validation_epoch: Optional[int] = None,
                 hybrid: bool = False,):
    ...
    if num_gpus > 1:
        ...
        nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, trainer_class_name,
                                               plans_identifier, use_compressed_data, device=device, hybrid=hybrid)
        ...
        maybe_load_checkpoint(nnunet_trainer, continue_training, only_run_validation, pretrained_weights, validation_epoch)
...
def run_training_entry():
    ...
    parser.add_argument('-epoch', type=int, default=None, required=False,
                        help='the validation epoch')
    parser.add_argument('-hybrid', action='store_true', required=False,
                        help='use the hybrid loss or not')
    ...
    run_training(args.dataset_name_or_id, args.configuration, args.fold, args.tr, args.p, args.pretrained_weights,
                 ...
                 device=device, validation_epoch=args.epoch, hybrid = args.hybrid)
